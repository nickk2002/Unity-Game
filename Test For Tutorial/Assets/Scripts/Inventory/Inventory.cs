...
public class Inventory : MonoBehaviour
{
    ...
    private void DisplayImage(int index,Weapon weapon, InventoryItem.ItemType type, GameObject reference)
    {
        ...
        slotImage.GetComponent<InventoryItem>().Reference = reference;
    ...
    public void AddItem(Weapon weapon,InventoryItem.ItemType type,GameObject reference)
    {
        if (weapon.itemSprite != null) {
            DisplayImage(enabledSlots, weapon, type,reference);
            Items.Add(weapon);
            ++enabledSlots;
        }   
    }
    public void RemoveItem(Weapon weapon)
    {
        ...
        foreach (Transform child in loot.transform)
        {
            if (child.childCount != 2)
                continue;
            GameObject slotText = child.GetChild(1).gameObject;
            if (slotText.GetComponent<Text>().text == weapon.itemName && weapon != null)
            {
                for (int i = 0; i < Items.Count; i++)
                {
                    if (Items[i].itemName == weapon.itemName)
